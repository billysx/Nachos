...
Thread::Thread(char* threadName, int pro = 5)
{
    name = threadName;
    stackTop = NULL;
    stack = NULL;
    ...
    for(int i=0;i<MAX_THREAD_ID;++i){
        if(USED_THREAD_ID[i] == 0){
            threadID = i;
            USED_THREAD_ID[i] = 1;
            //  printf("Allocated %s %d with priority %d\n",name,i,StaticPro);
            break;
        }
    }
    if(threadID == -1){
        printf("----------------------------------\n");
        printf("Thread number reaches the maximum.\n");
        printf("----------------------------------\n");
        ASSERT(0);
    }
...
//
//  Information printed including userID, threadID,
//  running status and thread name.
//----------------------------------------------------------------------
void
Thread::Print(){
    char MyStatusString[5][20] = { "JUST_CREATED", "RUNNING", "READY", "BLOCKED" };
    printf("Thread %d: name %s, userID %d, running status %s\n",threadID,name,userID, MyStatusString[status]);
}
#ifdef USER_PROGRAM
#include "machine.h"
